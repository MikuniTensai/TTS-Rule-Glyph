...
class _LevelSelectScreenState extends State<LevelSelectScreen> {
  ...
  List<String> get _chapterNames => campaignChapters;
  ...
  List<int> _getLevelIndicesForChapter(int chapIdx) {
    ...
    if (chapIdx == 0) {
      final int count = _chapterNames.length == 1 
          ? _levelList.length 
          : (BASE_LEVELS.length < _levelList.length ? BASE_LEVELS.length : _levelList.length);
      return List.generate(count, (i) => i);
    } else {
      final int startIdx = BASE_LEVELS.length + (chapIdx - 1) * ADVANCED_LEVELS.length;
      if (startIdx >= _levelList.length) {
        return [];
      }
      final int count = chapIdx == _chapterNames.length - 1
          ? _levelList.length - startIdx
          : (ADVANCED_LEVELS.length < _levelList.length - startIdx ? ADVANCED_LEVELS.length : _levelList.length - startIdx);
      return List.generate(count, (i) => startIdx + i);
    }
  }
